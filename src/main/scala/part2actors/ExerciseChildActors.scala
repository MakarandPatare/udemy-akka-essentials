...
object ExerciseChildActors extends App {
  ...
  class WordCounterMaster extends Actor {
    ...
    def withChildren(childrenRefs: Seq[ActorRef], currentChildIndex: Int, currentTaskId: Int, requestMap: Map[Int, ActorRef]): Receive = {
      case text: String =>
        ...
        context.become(withChildren(childrenRefs, (currentChildIndex + 1) % childrenRefs.length, currentTaskId + 1, newRequestMap))
   ...
   *    - 10 WordCounterWorker wil lbe created
   ...
   *
   *  requester -> WordCounterMaster -> WordCounterWorker -> WordCounterMaster -> requester
   */
